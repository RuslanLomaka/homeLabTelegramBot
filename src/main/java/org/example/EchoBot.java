...
public class EchoBot {
                    ...
                    for (Update update : updates) {
                        if (update.hasMessage() && update.getMessage().hasText()) {
                            ...
                            if (text.equals("/start")) {
                                sendMenu(chatId, telegramClient,
    ...
    private static void handleAgeMode(long chatId, String text, OkHttpTelegramClient client) {
        ...
        try {
            if (s.year == null) {
                ...
                if (year > currentYear)
                    throw new IllegalArgumentException("❌ Year is in the future.");
                if (year < currentYear - 200)
                    throw new IllegalArgumentException("❌ That’s over 200 years ago!");
                s.year = year;
    ...
    private static Integer monthFromName(String text) {
        ...
        for (int i = 0; i < names.length; i++)
            if (t.contains(names[i])) return i + 1;
        return null;
    }
